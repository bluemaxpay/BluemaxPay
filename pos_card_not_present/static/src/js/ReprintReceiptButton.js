odoo.define('pos_card_not_present.ReprintReceiptButton', function(require) {
    'use strict';

    const ReprintReceiptButton = require('point_of_sale.ReprintReceiptButton');
    const Registries = require('point_of_sale.Registries');

    const ReprintReceiptButtonExtended = (ReprintReceiptButton) =>
        class extends ReprintReceiptButton {
            async _onClick() {
                if (!this.props.order) return;
                const paymentDataArray = this.env.pos.pos_payment_data;
    ...
    Registries.Component.extend(ReprintReceiptButton, ReprintReceiptButtonExtended);
