...
class BluemaxPosPayment(models.Model):
    ...
    payment_method_id = fields.Many2one('pos.payment.method', string='POS Payment Method', required='True')
    state = fields.Selection([('draft', 'Draft'), ('success', 'Success'), ('failed', 'Failed')], string='Status', default='draft')
    ...
    def create_bluemax_payment(self, data):
        data.update({'currency_id': self.env.ref('base.%s' % data.get('currency_id')).id})
