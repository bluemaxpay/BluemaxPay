...
class PosBluemax(http.Controller):
    ...
    def fetch_bluemax_data(self, **kw):
        ...
        session = request.env['pos.session'].sudo().search([('state', '=', 'opened'), ('user_id', '=', int(kw.get('uid')))], limit=1)
        ...
        request.env['bus.bus'].sudo()._sendmany([[
            session.user_id.partner_id, 'bluemax.pos.payment.channel', {'payment_response': kw}
        ]])
