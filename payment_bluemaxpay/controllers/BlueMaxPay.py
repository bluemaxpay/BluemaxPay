...
class BlueMaxPayController(http.Controller):
    ...
    def test_simulate_payment(self, reference, bluemaxpay_transaction):
        _logger.info("received bluemaxpay return data:\n%s", pprint.pformat(reference))
        bluemaxpay_trans = request.env['bluemaxpay.transaction'].sudo().browse(bluemaxpay_transaction)
    ...
    def bluemaxpay_return_from_redirect(self, **data):
        ...
        tx_sudo = request.env['payment.transaction'].sudo()._get_tx_from_notification_data('bluemaxpay', data)
    ...
    def bluemaxpay_sale_order(self, **post):
        ...
        if sale_order_id:
            ...
            trans_id = request.session.get('__payment_monitored_tx_ids__')
            invoice = True
    ...
    def transaction_payment_create(self, **code):
        ...
        response = ''
        if code.get('trans_id'):
            if code.get('is_invoice'):
            ...
            trans_id = None
        sale = request.env['sale.order'].sudo().browse(code.get('sale'))
        try:
        ...
        if code.get('is_card'):
            token = request.env['bluemax.token'].browse(int(code.get('card')))
            card = CreditCardData()
            card.token = token.token
        else:
            try:
                card = CreditCardData()
                card.number = code.get('number')
                card.exp_month = code.get('exp_month')
                card.exp_year = code.get('exp_year')
                card.cvn = code.get('card_code')
                card.card_holder_name = code.get('name')
            except ApiException as e:
                return {
                    'error_message': True,
                    'message': e
